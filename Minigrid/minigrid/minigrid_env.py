...
class MiniGridEnv(gym.Env):
    ...
    def _reward(self, para) -> float:
        """
        Compute the reward to be given upon success
        """

        #return 1 - 0.9 * (self.step_count / self.max_steps)
        return para + 10 * ((500 - self.step_count) / 500)
    ...
    def step(
        ...
        # Pick up an object
            ...
            if fwd_cell and fwd_cell.can_pickup():
                if self.carrying is None:
                    ...
                    if fwd_cell.type == "box":      #YEEES bien joué, poto !!
                        terminated = True
                        reward += self._reward(15)
                    if not fwd_cell.already_pickup:
                        fwd_cell.already_pickup = True
                        reward += self._reward(4)
                    else:       #pickup un objet déja pris par le passé
                        reward -= 10
                else:           #pickup alors qu'il a déja quelque chose dans les mains
                    reward -= 2
            else:
                reward -= 2
        ...
        # Drop an object
            ...
            if not fwd_cell and self.carrying:
                ...
                self.carrying = None
            if not self.carrying:       #drop alors qu'on a aucun objet sur nous
                reward -= 2
        ...
        # Toggle/activate an object
            ...
            if fwd_cell:
                ...
                if fwd_cell.type == "door" and fwd_cell.is_open == True:
                    if not fwd_cell.already_open:
                        reward += self._reward(6)
                        fwd_cell.already_open = True
                    else:
                        reward -= 2
